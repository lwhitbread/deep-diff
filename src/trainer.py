...
class Trainer: 
    def __init__(
        ...
        synth_dataset: bool = False,
        # use_cortical_mask: bool = False,
        # blur_mask_args: dict = None,
        ...
        # self.use_cum_tracker = use_cum_tracker

        # self.use_cortical_mask = use_cortical_mask
        # self.blur_mask_args = blur_mask_args

        # self.mean_outside_loop = mean_outside_loop
        ...
        # if compile_flag:
        #     self.network_execute = torch.compile(self.network_execute)
            # if zero_mean_cons:
    ...
    def run(
        ...
        # gen_intensity_loss = L1().loss
        # gen_intensity_loss_weight = .05 # default is 0.02
        # losses.append(gen_intensity_loss)
        # weights.append(gen_intensity_loss_weight)
                ...
                if self.intensity_field_multiplier_st_1 == 0 and self.intensity_field_multiplier_st_2 == 0 and \
                    ...
                    if isinstance(self.intensity_field_multiplier_st_1, (int, float)):
                        if self.intensity_field_multiplier_st_1 == 0 and self.intensity_field_multiplier_st_2 == 0:
                            ...
                            # self.weights[-3] = self.loss_weights["int_1"]
                            # self.weights[-2] = self.loss_weights["int_2"]
                            # print("Intensity field weights and intensity modulation are active")
                        ...
                        if self.intensity_field_multiplier_st_1[epoch] == 0 and self.intensity_field_multiplier_st_2[epoch] == 0:
                            ...
                            # if freeze_int_weights:
                            #     print("Intensity field weights and intensity modulation are active")
                            # freeze_int_weights = False
                            # self.weights[-3] = self.loss_weights["int_1"]
                            # self.weights[-2] = self.loss_weights["int_2"]
                                                
                        # self.int_field_mult_fwd = self.intensity_field_multiplier[epoch]
    ...
    def _do_epoch(
        ...
        if (epoch + 1) % 5 == 0:
            print(f"  {epoch_type} loss (elements): \n \
                    ...
                    # \tintensity_field_1_max: {int_1_max}; \n \
                    # \tintensity_field_1_min: {int_1_min}; \n \
                    # \tintensity_field_2_max: {int_2_max}; \n \
                    # \tintensity_field_2_min: {int_2_min} \n \
                    # \tintensity_field_1_mean: {int_1_mean}; \n \
                    # \tintensity_field_2_mean: {int_2_mean} \n"
                    ...
                    #         "cond_temp" : pred[-1][0].detach().cpu().numpy(),
    ...
    def _network_loop(
        ...
        for idx_0, batch in tqdm(
            ...
            int_2_mean = _int_2_mean * curr_mean_prop + int_2_mean * old_mean_prop
                    ...
                    # _results = {
                    #     "scan_id": scan_id[idx_2],
                    #     "scan_path": scan_path[idx_2],
                    #     "params": params[idx_2].detach().cpu().numpy(),
                    #     "params_orig": params_orig[idx_2],
                    #     "pred_sample": pred[0][idx_2].detach().cpu().numpy(),
                    #     "stage_2_warp": pred[2][idx_2].detach().cpu().numpy(),
                    #     "image": images[idx_2].detach().cpu().numpy(),
                    #     "cond_temp" : pred[5][idx_2].detach().cpu().numpy(),
                    #     "cond_temp_warp" : pred[1][idx_2].detach().cpu().numpy(),
                    #     "cond_temp_unbiased" : pred[6][idx_2].detach().cpu().numpy() if pred[6] is not None else None,
                    #     "orientation" : orientation[idx_2],
                    #     "geometry" : geometry[idx_2],
                    #     "vox_size" : vox_size[idx_2],
                    #     "losses": lost_list, # with batches greater than 1, the losses are averaged
                    #     "total_loss": loss.item(), # with batches greater than 1, the losses are averaged
                    # }
    ...
    def _network_execute(
        ...
        with torch.autocast(
            ...
            if epoch_type == "train":
                if not self.zero_mean_cons:
                    pred = self.model(
                        ...
                        # epoch = epoch,
                        # cortical_mask = template_seg_tensor,
                    )
                else:
                    pred = self.model(
                        params, 
            ...
            # cortical_mask = pred[-3]

            # if cortical_mask is not None and not self.blur_mask_args["blur_mask"]:
            #     if epoch == 0 and itr == 0:
            #         print("Adjusting cortical mask for region volumes")
                
            #     total_mask_vol = torch.count_nonzero(cortical_mask != 0, dim = tuple(range(1, self.nb_dims + 2)))
                
            #     subcort_weight = self.loss_args["region_weights"]["subcortex"] * \
            #         total_mask_vol / torch.count_nonzero(cortical_mask == 1, dim = tuple(range(1, self.nb_dims + 2)))
            #     cortex_weight =  self.loss_args["region_weights"]["cortex"] * \
            #         total_mask_vol / torch.count_nonzero(cortical_mask == 2, dim = tuple(range(1, self.nb_dims + 2)))
            #     anchor_weight = self.loss_args["region_weights"]["anchor"] * \
            #         total_mask_vol / torch.count_nonzero(cortical_mask == 3, dim = tuple(range(1, self.nb_dims + 2)))

                
            #     for idx_i in range(cortical_mask.shape[0]):
            #         cortical_mask[idx_i] = torch.where(cortical_mask[idx_i] == 1, subcort_weight[idx_i], cortical_mask[idx_i])
            #         cortical_mask[idx_i] = torch.where(cortical_mask[idx_i] == 2, cortex_weight[idx_i], cortical_mask[idx_i])
            #         cortical_mask[idx_i] = torch.where(cortical_mask[idx_i] == 3, anchor_weight[idx_i], cortical_mask[idx_i])
...
#         #     **self.dataset_args,
#         # )
#         dataloader_train = self.dataloaders[0]
#         self.len_train_loader = len(dataloader_train)
#         dataloader_val = self.dataloaders[1]
#         self.len_val_loader = len(dataloader_val)
#         if len(self.dataloaders) == 3:
#             dataloader_test = self.dataloaders[2]
#             self.len_test_loader = len(dataloader_test)
...
#         # vector field reconstruction loss
#         self.loss = VectorLoss(
#             magnitude_weight = self.loss_weights["vec_magnitude"],
#             cosine_weight= self.loss_weights["vec_cosine"],
#             magnitude_penalty = self.loss_weights["vec_magnitude_penalty"],
#         ).loss
#         print(f"loss magnitude weight: {self.loss_weights['vec_magnitude']}, cosine weight: {self.loss_weights['vec_cosine']}")
#         print(f"loss magnitude_penalty: {self.loss_weights['vec_magnitude_penalty']}")
#         # self.losses.append(vector_field_loss)
...
#             if epoch == 0:
#                 if self.run_monitor_args is not None:
#                     _dir_path = f"{self.runtime_dir}/../models/{self.run_monitor_args['id']}/inverse_warp_map/"
#                     if not os.path.exists(_dir_path):
#                         os.mkdir(_dir_path)
#                     _len_dir_path = len([entry for entry in os.listdir(_dir_path) if os.path.isfile(os.path.join(_dir_path, entry))])
#                     if _len_dir_path > 0:
#                         raise(FileExistsError(f"Directory {self.runtime_dir}/../models/{self.run_monitor_args['id']}/inverse_warp_map \
#                                 is not empty. Please delete the contents of this directory before running the training."))
#                     _dir_path = f"{self.runtime_dir}/../results/{self.run_monitor_args['id']}/inverse_warp_map/"
#                     self.dir_path_history_data = f"{self.runtime_dir}/../results/{self.run_monitor_args['id']}/inverse_warp_map/"
#                     if not os.path.exists(_dir_path):
#                         os.makedirs(_dir_path)
#                     if not os.path.exists(self.dir_path_history_data):
#                         os.makedirs(self.dir_path_history_data)
#                     _len_dir_path = len([entry for entry in os.listdir(_dir_path) if os.path.isfile(os.path.join(_dir_path, entry))])
#                     if _len_dir_path > 0:
#                         raise(FileExistsError(f"Directory {self.runtime_dir}/../results/{self.run_monitor_args['id']}/inverse_warp_map \
#                                 is not empty. Please delete the contents of this directory before running the training."))
...
#             # self.history["train"]["loss_vel"].append(loss_tup[1][0])
#             # self.history["train"]["magnitude_loss_vel"].append(loss_tup[1][1])
#             # self.history["train"]["cosine_loss_vel"].append(loss_tup[1][2])
#             # self.history["train"]["loss_warp"].append(loss_tup[3][0])
#             # self.history["train"]["magnitude_loss_warp"].append(loss_tup[3][1])
#             # self.history["train"]["cosine_loss_warp"].append(loss_tup[3][2])
#             # self.history["train"]["epoch"].append(epoch + 1)
#             # self.history["train"]["time"].append(time.time() - start)
#             # self.history["train"]["learning_rate"].append(self.optimiser.param_groups[0]['lr'])
...
#             warp_field = batch["cond_temp_warp"]
#             velocity_field_gt = batch["flow_st_1"]
#             curr_batch_size = warp_field.shape[0]
#             warp_field = warp_field.to(self.device)
#             velocity_field_gt = velocity_field_gt.to(self.device)
...
#             ):
#                 if epoch_type == "train":
#                     preds = self.model(warp_field)
#                 else:
#                     with torch.no_grad():
#                         preds = self.model(warp_field)

#                 losses = self.loss(preds[0], velocity_field_gt)
#                 loss = losses[0]
#                 eval_losses = self.loss(preds[1], warp_field)
...
#                 #                 "velocity_field_gt": velocity_field_gt.detach().cpu().numpy(),
#                 #                 "pred": preds[0].detach().cpu().numpy(),
#                 #                 "pred_inv": preds[1].detach().cpu().numpy(),
#                 #             }
#                 #         )
...
#                 if self.mixed_precision:
#                     self.scaler.scale(loss).backward()
#                     for idx_1 in range(len(losses)):
#                         self.scaler.scale(losses[idx_1])
#                         self.scaler.scale(eval_losses[idx_1])
#                 else: 
#                     loss.backward()
                
#                 if (idx_0 + 1) % self.gradient_accumulation_steps == 0:
#                     if self.clip_grad:
#                         if self.mixed_precision:
#                             self.scaler.unscale_(self.optimiser)
#                         torch.nn.utils.clip_grad_norm_(self.model.parameters(), self.clip_grad_norm)
...
#         # epoch_total_loss = np.mean(epoch_total_loss)
#         mean_epoch_loss = np.mean(epoch_loss, axis = 0)
#         # epoch_total_loss_eval = np.mean(epoch_total_loss_eval)
#         mean_epoch_loss_eval = np.mean(epoch_loss_eval, axis = 0)

#         self.history[epoch_type]["loss_vel"].append(mean_epoch_loss[0])
#         self.history[epoch_type]["magnitude_loss_vel"].append(mean_epoch_loss[1])
#         self.history[epoch_type]["cosine_loss_vel"].append(mean_epoch_loss[2])
#         self.history[epoch_type]["loss_warp"].append(mean_epoch_loss_eval[0])
#         self.history[epoch_type]["magnitude_loss_warp"].append(mean_epoch_loss_eval[1])
#         self.history[epoch_type]["cosine_loss_warp"].append(mean_epoch_loss_eval[2])
#         self.history[epoch_type]["epoch"].append(epoch + 1)
#         self.history[epoch_type]["time"].append(time.time() - start)
#         self.history[epoch_type]["learning_rate"].append(self.optimiser.param_groups[0]['lr'])
        
        
#         print(f"  {epoch_type} total loss: {mean_epoch_loss[0]}, magnitude loss: {mean_epoch_loss[1]}, cosine loss: {mean_epoch_loss[2]}")
#         print(f"  {epoch_type} total loss eval: {mean_epoch_loss_eval[0]}, magnitude loss eval: {mean_epoch_loss_eval[1]}, cosine loss eval: {mean_epoch_loss_eval[2]}")

#         if epoch_type == "val":
#             return mean_epoch_loss[0]
...
#         if best:
#             if self.best_model_name is not None:
#                 os.system(f"rm {self.runtime_dir}/../models/{self.run_monitor_args['id']}/inverse_warp_map/{self.best_model_name}")
            
#             self.best_model_name = f"best_model_run_{self.run_monitor_args['id']}_epoch_{self.best_epoch}_val_loss_{self.best_val_loss:.6f}_time_{time.strftime('%Y-%m-%d_%H.%M.%S')}.pt"
#             torch.save(
#                 self.model.state_dict(),
#                 f"{self.runtime_dir}/../models/{self.run_monitor_args['id']}/inverse_warp_map/{self.best_model_name}",
#             )

#         if last:
#             torch.save(
#                 self.model.state_dict(), 
#                 f"{self.runtime_dir}/../models/{self.run_monitor_args['id']}/inverse_warp_map/final_model_run_{self.run_monitor_args['id']}.pt",
#             )
...
#         _df_val.columns = ["val_" + col for col in _df_val.columns]
#         _df = pd.concat([_df_train, _df_val], axis = 1)
#         _df.to_csv(f"{self.runtime_dir}/../results/{self.run_monitor_args['id']}/inverse_warp_map/history_run_{self.run_monitor_args['id']}.csv", index = False)